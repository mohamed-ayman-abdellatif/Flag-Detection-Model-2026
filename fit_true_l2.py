...
def loss_function_l2(params, data_points, W, H, f, phi):
...
def main():
    ...
    phi_deg_sweep = np.linspace(-180.0, 180.0, 361) # 1 degree steps
    results = []
    ...
    for phi_deg in phi_deg_sweep:
        ...
        for alt_g_val in np.linspace(alt0 - 150.0, alt0 - 20.0, 27):
            ...
            loss = loss_function_l2([xy_f[1], xy_f[0], alt_g_val], data_points, W, H, f, phi_rad)
            if loss < best_loss:
                best_loss = loss
        ...
        res = opt.minimize(loss_function_l2, guess, args=(data_points, W, H, f, phi_rad), bounds=bounds, method='L-BFGS-B')
        if res.success:
    ...
    print("\nTop 10 configurations (True L2 Loss):")
    ...
    print(f"\nBest Config Lat/Lon from moving sequence (True L2):")
