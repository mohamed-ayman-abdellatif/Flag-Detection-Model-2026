...
def main():
    ...
    # Compute headings from velocity vector
    headings = []
    ...
    f = 3000.0 * (W / 3840.0)
    print(f"Video Resolution: {W}x{H}, Focal Length: {f}")
    ...
    # We will test phi from -180 to 180 degrees in steps of 5 degrees
    phi_deg_sweep = np.linspace(-180.0, 180.0, 73)
    results = []
    
    print("\nSweeping camera yaw offset (phi)...")
    for phi_deg in phi_deg_sweep:
        phi_rad = np.radians(phi_deg)
        
        # Grid search initialization for ground alt
        best_loss = 1e15
        best_alt_g = None
        for alt_g_val in np.linspace(alt0 - 150.0, alt0 - 20.0, 27):
            # linear least squares for N_f, E_f
            A_rows = []
            B_rows = []
            for dp in data_points:
